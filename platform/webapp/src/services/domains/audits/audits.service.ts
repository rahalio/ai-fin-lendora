...
const rawAuditsService = {
   ...
   */
  async getAudit(params?: Record<string, any>, signal?: AbortSignal): Promise<any> {
    const orgId = getEffectiveOrgId();
    if (!orgId) {
      throw new Error("Organization ID is required");
    }

    const url = `/orgs/${orgId}//v1/audits` + (params ? `?${new URLSearchParams(params).toString()}` : '');
   ...
   */
  async createAudit(data?: any, signal?: AbortSignal): Promise<any> {
    const orgId = getEffectiveOrgId();
    if (!orgId) {
      throw new Error("Organization ID is required");
    }

    const url = `/orgs/${orgId}//v1/audits`;
   ...
   * Get Decision Audit
   */
  async getAudit(auditId: string, params?: Record<string, any>, signal?: AbortSignal): Promise<any> {
    const orgId = getEffectiveOrgId();
    if (!orgId) {
      throw new Error("Organization ID is required");
    }

    const url = `/orgs/${orgId}//v1/audits/${auditId}` + (params ? `?${new URLSearchParams(params).toString()}` : '');
   ...
   */
  async getByApplication(applicationId: string, params?: Record<string, any>, signal?: AbortSignal): Promise<any> {
    const orgId = getEffectiveOrgId();
    if (!orgId) {
      throw new Error("Organization ID is required");
    }

    const url = `/orgs/${orgId}//v1/audits/by-application/${applicationId}` + (params ? `?${new URLSearchParams(params).toString()}` : '');
