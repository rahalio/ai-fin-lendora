...
const DOMAIN_PATH_PATTERNS: Array<{ pattern: RegExp; domain: string }> = [
  ...
  { pattern: /\/applications/, domain: "applications" },
  { pattern: /\/audits/, domain: "audits" },
  { pattern: /\/early-warnings/, domain: "early-warnings" },
  { pattern: /\/models/, domain: "models" },
  { pattern: /\/learning-envelopes/, domain: "models" },
  { pattern: /\/policies/, domain: "policies" },
  { pattern: /\/reason-codes/, domain: "policies" },
  { pattern: /\/refer-tasks/, domain: "referrals" },
];
...
export async function injectDomainDependencies(
  ...
  if (!domainDeps) {
    ...
    switch (domain) {
      case "applications":
        domainDeps = buildApplicationsDomainModule(dynamoClient);
        break;
      case "audits":
        domainDeps = buildAuditsDomainModule(dynamoClient);
        break;
      case "early-warnings":
        domainDeps = buildEarlyWarningsDomainModule(dynamoClient);
        break;
      case "identity":
...
export interface DomainDependencies {
  applications: ApplicationsDomainModule;
  audits: AuditsDomainModule;
  "early-warnings": EarlyWarningsDomainModule;
  identity: IdentityDomainModule;
  models: ModelsDomainModule;
  policies: PoliciesDomainModule;
  referrals: ReferralsDomainModule;
}
 ...
 */
export function getApplicationsDeps(
  request: FastifyRequest,
): DomainDependencies["applications"] {
  return getDomainDeps(request, "applications");
}
...
export function getEarlyWarningsDeps(
  request: FastifyRequest,
): DomainDependencies["early-warnings"] {
  return getDomainDeps(request, "early-warnings");
}
...
export function getReferralsDeps(
  request: FastifyRequest,
): DomainDependencies["referrals"] {
  return getDomainDeps(request, "referrals");
}
