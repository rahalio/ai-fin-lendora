 ...
 * VModelRepositoryDdb
 *
 * DDD: Infrastructure adapter implementing VModel repository using DynamoDB.
 * Generated from x-dynamodb metadata in OpenAPI schema.
 */

import type { VModelRepository } from "@lendora/services/models";
...
/**
 * VModelRepositoryDdb using DynamoDB table (type: base)
 *
 * Entity Type: MODEL_VERSION
 * Entity Token: MODEL_VERSION
 * SK Prefix: MODEL_VERSION
 */
export class VModelRepositoryDdb implements VModelRepository {
  ...
  private readonly ENTITY_TYPE = "MODEL_VERSION";
  private readonly ENTITY_TOKEN = "MODEL_VERSION";
  private readonly SK_PREFIX = "MODEL_VERSION";
  ...
  private readonly DOMAIN = "MODELS";
  private readonly ENTITY_TYPE_PK = "MODEL_VERSIONS";
  private generateCorrelationId(): string {
    const domainPrefix = "mdl";
  ...
  async getCreditModel(input: Parameters<VModelRepository['getCreditModel']>[0]): Promise<Awaited<ReturnType<VModelRepository['getCreditModel']>>> {
    ...
    if (!(inputExt.id ?? inputExt.modelVersionId)) {
      throw new Error("Missing required parameter: id");
    }
    if (!inputExt.orgId) {
      throw new Error("Missing required parameter: orgId");
    }
    const pk = this.buildPK(inputExt.orgId as string, (inputExt.id ?? inputExt.modelVersionId) as string);
    // Soft-delete layout prefixes SK with ACTIVE# when statusPrefix is enabled.
    const skPrefix = `${this.ENTITY_TOKEN}#${(inputExt.id ?? inputExt.modelVersionId)}#`;
    const result = await this.dynamoClient.query({
      TableName: this.TABLE_NAME,
    ...
    if (!item) {
      throw new Error(`${this.ENTITY_TYPE} with id ${(inputExt.id ?? inputExt.modelVersionId)} not found`);
    }
    return {
      data: sanitizeItem(item as Record<string, unknown>) as unknown as any,
      meta: {
