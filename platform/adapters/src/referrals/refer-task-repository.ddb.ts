/**
 * ReferTaskRepository - DynamoDB Implementation
 *
...
/**
 * ReferTaskRepositoryDdb
 *
 * DDD: Infrastructure adapter implementing ReferTask repository using DynamoDB.
 * Generated from x-dynamodb metadata in OpenAPI schema.
 */

import type { ReferTaskRepository } from "@lendora/services/referrals";
...
/**
 * ReferTaskRepositoryDdb using DynamoDB table (type: core)
 *
 * Entity Type: REFER_TASK
 * Entity Token: REFER_TASK
 * SK Prefix: REFER_TASK
 */
export class ReferTaskRepositoryDdb implements ReferTaskRepository {
  private readonly TABLE_NAME: string;
  // Metadata from x-dynamodb annotation - all values are dynamic
  private readonly ENTITY_TYPE = "REFER_TASK";
  private readonly ENTITY_TOKEN = "REFER_TASK";
  private readonly SK_PREFIX = "REFER_TASK";
  ...
  private readonly ENTITY_TYPE_PK = "REFER_TASKS";
  ...
  async getReferTask(input: Parameters<ReferTaskRepository['getReferTask']>[0]): Promise<Awaited<ReturnType<ReferTaskRepository['getReferTask']>>> {
    ...
    if (!(inputExt.id ?? inputExt.referTaskId)) {
      throw new Error("Missing required parameter: id");
    }
    if (!inputExt.orgId) {
      throw new Error("Missing required parameter: orgId");
    }
    const pk = this.buildPK(inputExt.orgId as string, (inputExt.id ?? inputExt.referTaskId) as string);
    // Soft-delete layout prefixes SK with ACTIVE# when statusPrefix is enabled.
    const skPrefix = `${this.ENTITY_TOKEN}#${(inputExt.id ?? inputExt.referTaskId)}#`;
    const result = await this.dynamoClient.query({
      TableName: this.TABLE_NAME,
    ...
    if (!item) {
      throw new Error(`${this.ENTITY_TYPE} with id ${(inputExt.id ?? inputExt.referTaskId)} not found`);
    }
    return {
