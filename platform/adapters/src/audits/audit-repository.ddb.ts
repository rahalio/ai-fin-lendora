 ...
 * AuditRepositoryDdb
 *
 * DDD: Infrastructure adapter implementing Audit repository using DynamoDB.
 * Generated from x-dynamodb metadata in OpenAPI schema.
 */

import type { AuditRepository } from "@lendora/services/audits";
...
/**
 * AuditRepositoryDdb using DynamoDB table (type: core)
 *
 * Entity Type: DECISION_AUDIT
 * Entity Token: DECISION_AUDIT
 * SK Prefix: DECISION_AUDIT
 */
export class AuditRepositoryDdb implements AuditRepository {
  ...
  private readonly ENTITY_TYPE = "DECISION_AUDIT";
  private readonly ENTITY_TOKEN = "DECISION_AUDIT";
  private readonly SK_PREFIX = "DECISION_AUDIT";
  ...
  private readonly ENTITY_TYPE_PK = "DECISION_AUDITS";
  ...
  async getDecisionAudit(input: Parameters<AuditRepository['getDecisionAudit']>[0]): Promise<Awaited<ReturnType<AuditRepository['getDecisionAudit']>>> {
    ...
    if (!(inputExt.id ?? inputExt.auditId)) {
      throw new Error("Missing required parameter: id");
    }
    if (!inputExt.orgId) {
      throw new Error("Missing required parameter: orgId");
    }
    const pk = this.buildPK(inputExt.orgId as string, (inputExt.id ?? inputExt.auditId) as string);
    // Soft-delete layout prefixes SK with ACTIVE# when statusPrefix is enabled.
    const skPrefix = `${this.ENTITY_TOKEN}#${(inputExt.id ?? inputExt.auditId)}#`;
    const result = await this.dynamoClient.query({
      TableName: this.TABLE_NAME,
    ...
    if (!item) {
      throw new Error(`${this.ENTITY_TYPE} with id ${(inputExt.id ?? inputExt.auditId)} not found`);
    }
    return {
      data: sanitizeItem(item as Record<string, unknown>) as unknown as any,
      meta: {
