/**
 * ApplicationRepository - DynamoDB Implementation
 *
...
/**
 * ApplicationRepositoryDdb
 *
 * DDD: Infrastructure adapter implementing Application repository using DynamoDB.
 * Generated from x-dynamodb metadata in OpenAPI schema.
 */

import type { ApplicationRepository } from "@lendora/services/applications";
...
/**
 * ApplicationRepositoryDdb using DynamoDB table (type: core)
 *
 * Entity Type: APPLICATION
 * Entity Token: APPLICATION
 * SK Prefix: APPLICATION
 */
export class ApplicationRepositoryDdb implements ApplicationRepository {
  private readonly TABLE_NAME: string;
  // Metadata from x-dynamodb annotation - all values are dynamic
  private readonly ENTITY_TYPE = "APPLICATION";
  private readonly ENTITY_TOKEN = "APPLICATION";
  private readonly SK_PREFIX = "APPLICATION";
  ...
  private readonly ENTITY_TYPE_PK = "APPLICATIONS";
  ...
  async getApplication(input: Parameters<ApplicationRepository['getApplication']>[0]): Promise<Awaited<ReturnType<ApplicationRepository['getApplication']>>> {
    ...
    const pk = this.buildPK(inputExt.orgId as string, (inputExt.id ?? inputExt.applicationId) as string);
    // Soft-delete layout prefixes SK with ACTIVE# when statusPrefix is enabled.
    const skPrefix = `${this.ENTITY_TOKEN}#${(inputExt.id ?? inputExt.applicationId)}#`;
    const result = await this.dynamoClient.query({
      TableName: this.TABLE_NAME,
    ...
    if (!item) {
      throw new Error(`${this.ENTITY_TYPE} with id ${(inputExt.id ?? inputExt.applicationId)} not found`);
    }
    return {
      data: sanitizeItem(item as Record<string, unknown>) as unknown as any,
      meta: {
